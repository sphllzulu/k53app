...
class MockExamSelectionScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final level1Configs = MockExamConfig.level1Configs;

    // Group configs by category/type
    final allCategoryConfigs = level1Configs.where((config) => config.category == null).toList();
    final rulesConfigs = level1Configs.where((config) => config.category == 'rules_of_road').toList();
    final signsConfigs = level1Configs.where((config) => config.category == 'road_signs').toList();
    final controlsConfigs = level1Configs.where((config) => config.category == 'vehicle_controls').toList();

    return Scaffold(
      appBar: AppBar(
        title: const Text('Level 1 Mock Exams'),
        leading: IconButton(
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildInfoRow('Difficulty', 'Level 1 (Easy)'),
                  ],
                ),
              ),
            ),
